...
function openBoardModal(boardId) {
  ...
  if (!board) return;
  const height = window.currentWaveHeight;
  const period = window.currentWavePeriod;
  const windSpeed = window.currentWindSpeed;
  const windCondition = window.currentWindCondition;
  const boardNameElement =
    document.getElementById("modalBoardName");
  const conditionElement =
    document.getElementById("modalCondition");
  const adviceElement =
    document.getElementById("modalAdvice");
  const modalElement =
...
function getWindCondition(
  ...
  const offshoreFrom =
    Number.isFinite(spot.offshoreFrom)
      ? spot.offshoreFrom
      : 0;
  const offshoreRange =
    Number.isFinite(spot.offshoreRange)
      ? spot.offshoreRange
      : 30;
  const diff =
    angleDifference(
...
function drawTideGraph(
  ...
  const indexes = [
    0,
    Math.floor(
      (validPairs.length - 1) * 0.25
    ),
    Math.floor(
      (validPairs.length - 1) * 0.50
    ),
    Math.floor(
      (validPairs.length - 1) * 0.75
    ),
    validPairs.length - 1
  ];
  indexes.forEach(
...
async function loadData(
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `API request failed: marine=${marineResponse.status}, weather=${weatherResponse.status}`
      );
    }
    const marine =
